...
description: IsEqualTo 要素は、プロパティと定数値または別のプロパティを比較する検索式を表し、等しい場合は true と評価されます。
...
|[Or](or.md) <br/> |含まれる検索式に対して論理 OR を実行する検索式を表します。 [または](or.md) 、子が true を返す場合は true を返します。 **または、2** つ以上の子が必要です。  <br/> |
   
## <a name="remarks"></a>注釈

文字列比較を実行するには、大文字と小文字や空白など、一致するパラメーターのオプションを提供する [Contains](contains.md) 要素の使用を検討してください。 結果を [ネゲート](not.md) するには、Not 要素と [Contains](contains.md) 要素を組み合わせて使用します。 
