...
**Configurationrequestdetails**要素には、ポリシーシェイクのクライアントの状態が含まれています。 状態情報には、インストールされているルールとクライアントコンポーネントのバージョンを含めることができます。 
  
```XML
<ConfigurationRequestDetails></ConfigurationRequestDetails>
```

 **Configurationrequest/Stype**
...
子要素は、Exchange Web サービススキーマでは定義されていません。
