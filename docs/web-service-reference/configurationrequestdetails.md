...
**ConfigurationRequestDetails**要素には、微調整するポリシーのクライアントの状態が含まれています。 状態情報には、インストールされているルールとクライアント コンポーネントのバージョンを含めることができます。 
...
子要素は、Exchange Web サービスのスキーマで定義されていません。
