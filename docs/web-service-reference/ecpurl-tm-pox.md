...
**EcpUrl tm**の要素は、先のすべてのサイトのメールボックスのメールが有効なユーザーは、現在メンバー リストにアクセスするために使用できる URL を生成する[EcpUrl (POX)](ecpurl-pox.md)要素の値と組み合わせて使用できますが、部分的な URL を指定します。 
...
テキスト値は、ユーザーのメールボックスをサイトの一覧にアクセスするために使用できる URL を生成する[EcpUrl (POX)](ecpurl-pox.md)要素の値と組み合わせて使用できますが、部分的な URL を表します。 
  
## <a name="remarks"></a>備考

**EcpUrl tm**の要素は、**プロトコル**要素の省略可能な子要素です。 
