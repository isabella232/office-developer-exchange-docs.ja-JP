...
**Acceptitem**要素は、会議出席依頼への返信の承諾を表します。 
...
|[ProposedEnd](proposedend.md) <br/> |会議の終了時刻の提案を指定します。  <br/> |
...
この要素を記述するスキーマは、クライアントアクセスサーバーの役割がインストールされている Exchange サーバーの EWS ディレクトリにあります。
