...
**ItemClass**要素は、アイテムのメッセージ クラスを表します。 
...
テキスト値は、項目の型と一致する必要があります。 作成またはメールボックスの IPM メッセージを更新する場合などです。注または別のクラスは、メッセージと一致する必要があります。 空の値を指定することはできません。 空の型が有効なを作成するまたは、アイテムを更新する場合。
  
エラーが返される**ItemClass**が項目の種類と一致している値に設定されている場合。 たとえば、メッセージの**ItemClass**は、タスクの**ItemClass**値に設定できません。 
