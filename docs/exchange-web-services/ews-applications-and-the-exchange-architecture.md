...
EWS が Exchange アーキテクチャで動作するしくみと、EWS が依存するプロトコルについて説明します。
  
により、Exchange、オンラインの Office 365 の一部として Exchange Online 電子メール メッセージ、会議、連絡先などのメールボックスのアイテムにアクセスするアプリケーションまたは社内設置型 Exchange からのクロス ・ プラットフォーム API は、Exchange Web サービス (EWS)Exchange Server 2007。 [EWS アプリケーション](ews-application-types.md)でアクセスできますメールボックス アイテムをローカルまたはリモートで SOAP ベースの XML メッセージの要求を送信します。 SOAP メッセージは、アプリケーションと、アプリケーションが HTTP 経由の XML を投稿できる場合に限り使用できる EWS Exchange にアクセスすることを意味すると、サーバー間で送信されるときに、HTTP メッセージに埋め込まれます。 
...
**図 1 です。EWS アプリケーションと Exchange の設置型のアーキテクチャ**
...
**図 2 になります。EWS アプリケーションと Exchange Online のアーキテクチャ**
...
1. EWS アプリケーション: これは、[クライアント、ポータル、またはサービス アプリケーション](ews-application-types.md)とクライアントまたは Exchange、オンプレミスのクライアント アクセス サーバー上にインストールすることができます。 EWS アプリケーションを開発するのには、EWS のマネージ API を使用する場合、クライアントと[、アプリケーションで再配布](redistribution-requirements-for-the-ews-managed-api.md)をインストールする必要は EWS のマネージ API のアセンブリです。
...
5. クライアント アクセス サーバー アレイ、クライアント アクセス サーバーは、負荷分散されたクライアント アクセス サーバー アレイと呼ばれるグループに編成します。 個々 のクライアント アクセス サーバーは、認証、制限付きのリダイレクト、およびプロキシ サービスを提供します。 クライアント アクセス サーバーとは、データの表示を行わないとデータがキューに入れられたまたはクライアント アクセス サーバーに格納されているが、シンと状態がありません。だけで、要求を認証する、自動検出を参照し、[プロキシのメールボックス サーバーに要求を実行します。 クライアント アクセス サーバーはユーザーのデータをホストしているメールボックス サーバーと 1 対 1 のリレーションシップを管理します。 (自己署名証明書を使用して SSL 経由でセキュリティで保護された) HTTP プロトコルは、クライアント アクセス サーバーとメールボックス サーバー間で使用されます。 このコンポーネントは、Exchange の設置型のアーキテクチャでのみ表示されます。
    
6. 自動検出サービスなどの自動検出サービスは、Active Directory ドメイン サービス (AD DS)、メールボックスのバージョンとユーザーのデータのアクティブ コピーをホストしているメールボックス サーバーの場所を取得するにアクセスすることでサービス検出を実行します。
...
7. EWS サービス - EWS サービスは、次の 3 つのファイルで記述されます。Services.wsdl、Messages.xsd、Types.xsd、および EWS マネージ API のアセンブリです。Services.wsdl はクライアントとサーバーの間のコントラクトを記述し、Messages.xsd は要求および応答の SOAP メッセージを定義し、Types.xsd は SOAP メッセージで使用される要素を定義します。以前のバージョンのスキーマが存在する場合でも、Messages.xsd と Types.xsd には常に最新バージョンのスキーマが含まれます。Services.wsdl、Messages.xsd、Types.xsd はクライアント アクセス サーバーで利用可能ですが、実際にはスキーマの検証で使用されないことに注意してください。これらは参照目的でのみ提供されます。EWS マネージ API アセンブリは、サーバー側の EWS クライアント アプリケーションに提供され、クライアント アクセス サーバーだけでなく、すべての Exchange Server の役割で展開されます。このコンポーネントは、Exchange オンプレミス アーキテクチャでのみ表示されます。
    ...
    EWS のスキーマのバージョンに基づいて使用可能な機能をアプリケーションがターゲットです。 アプリケーションの動作にも、後でスキーマのバージョン、Exchange 2010 SP2 サービスなどに対して Exchange 2007 SP1 など、以前のスキーマ バージョンを対象とするアプリケーションを作成する場合、EWS のスキーマは後方と前方の互換性があるためだけでなくオンラインを交換します。 機能および機能の更新は、スキーマによって推進されています、ため、クライアント アプリケーションに実装する EWS 機能を対象とする最も一般的なコード ベースを使用することをお勧めします。 多くのアプリケーションは、Exchange 2007 SP1 のスキーマには、ほとんどすべて、中核となる Exchange ストアのアイテムとフォルダーを操作するための Exchange の機能が含まれているために、Exchange2007_SP1 バージョンを対象できます。 詳細については、 [EWS のクライアントの機能](ews-client-design-overview-for-exchange.md#EWSFeatures)を参照してください。
...
9. オンラインの Exchange および Exchange オンライン Office 365 の一部として、クラウド ・ ベースのサービスと Exchange の機能を提供するホストされているメッセージング ソリューションです。
...
- OAuth 2.0 トークン認証、信頼されたパートナーのアプリケーションと Lync Server 2013 と SharePoint Server 2013 には、相互運用性。
